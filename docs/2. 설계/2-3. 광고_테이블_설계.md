...
erDiagram
    ...
    ad_campaigns ||--o{ ad_rank_calculations : "캠페인별 AD_RANK 계산 결과"
    ...
    ad_products_meta ||--o{ ad_rank_calculations : "상품별 AD_RANK 계산 결과"
    ...
    %% AD_RANK 계산 결과 테이블
    ad_rank_calculations ||--o{ ad_rank_calculations : "카테고리별 AD_RANK 계산"
    ...
    ad_rank_calculations {
        bigint id PK "계산 ID"
        bigint campaign_id FK "캠페인 ID"
        bigint product_id FK "상품 ID"
        varchar placement "광고 영역"
        varchar category_path "카테고리 경로"
        decimal ad_rank_score "AD_RANK 점수"
        decimal cpc_price "CPC 가격"
        int exposure_rank "노출 순위"
        decimal exposure_weight "노출 비중"
        decimal random_weight "랜덤 노출 비중"
        varchar calculation_status "계산 상태"
        timestamp calculated_at "계산 시간"
        tinyint activated "활성화 여부"
        datetime created_at "등록일시"
        datetime updated_at "수정일시"
        bigint created_user_id "생성자 ID"
...
> 
> **핵심 개선**: AD_RANK 계산 로직은 DB, 실시간 랭킹 조회는 엘라스틱서치, 분석용 원시 데이터는 S3
...
| **운영 관리** | 4개 | `ad_daily_budget_trackings`, `ad_product_status_caches`, `ad_impression_pricings`, `ad_rank_calculations` | 예산 추적, 성과 캐시, AD_RANK 가격 매핑, AD_RANK 계산 결과 |
...
**총 16개 테이블**로 구성된 AD_RANK 기반 효율적인 데이터 아키텍처
...
> **목적**: 클릭 시 차감할 정확한 AD_RANK 기반 가격 정보 저장
...
CREATE TABLE ad_impression_pricings (
    ...
    ad_rank          INT                                            NOT NULL                    COMMENT 'AD_RANK 순위 (1등, 2등, 3등...)',
    algorithm_score  DECIMAL(10,4)                                  NOT NULL                    COMMENT 'AD_RANK 점수',
...
) ENGINE=InnoDB COMMENT='노출별 AD_RANK 가격 매핑';
```

---

### 10. AD_RANK 계산 결과 (ad_rank_calculations)
> **목적**: AD_RANK 알고리즘 계산 결과 저장 및 캐싱
> **비즈니스 요구사항**: 실시간 랭킹 조회, 엘라스틱서치 동기화, 계산 이력 추적

**📋 데이터 Flow:**
- **INSERT**: 
  - Worker가 AD_RANK 계산 완료 시 결과 저장
  - 카테고리별 전체 상품의 AD_RANK, CPC, 노출 순위, 노출 비중 저장
  - 엘라스틱서치 인덱스와 동기화
- **UPDATE**: 
  - 캠페인 변경 시 해당 카테고리 전체 재계산
  - 예산/잔액 소진 시 즉시 상태 업데이트
  - 계산 상태 변경 (CALCULATED/PAUSED/ERROR)
- **DELETE**: 삭제 없음, 계산 이력 보존

```sql
CREATE TABLE ad_rank_calculations (
    id               BIGINT                                         PRIMARY KEY AUTO_INCREMENT COMMENT 'AD_RANK 계산 결과 고유 식별자',
    campaign_id      BIGINT                                         NOT NULL                    COMMENT '캠페인 식별자 (FK)',
    product_id       BIGINT                                         NOT NULL                    COMMENT '상품 식별자 (FK)',
    placement        VARCHAR(20)                                    NOT NULL                    COMMENT '광고 영역 (HOME/CATEGORY/PRODUCT_DETAIL/SEARCH)',
    category_path    VARCHAR(200)                                   NOT NULL                    COMMENT '카테고리 경로',
    ad_rank_score    DECIMAL(15,4)                                  NOT NULL                    COMMENT 'AD_RANK 점수 (최대입찰가 × 클릭전환율)',
    cpc_price        DECIMAL(10,2)                                  NOT NULL                    COMMENT 'CPC 가격 (바로 아래 상품 AD_RANK / 클릭전환율 + 1)',
    exposure_rank    INT                                            NOT NULL                    COMMENT '노출 순위 (AD_RANK + 노출 기준치 가산점 + 구매 전환 가산점)',
    exposure_weight  DECIMAL(6,4)                                   NOT NULL                    COMMENT '노출 비중 (AD_RANK / 전체 합계 × 30% 제한)',
    random_weight    DECIMAL(6,4)                                   DEFAULT 0.0000              COMMENT '랜덤 노출 비중 (0.0000~1.0000)',
    calculation_status VARCHAR(20)                                  DEFAULT 'CALCULATED'         COMMENT '계산 상태 (CALCULATED/PAUSED/ERROR)',
    calculated_at    TIMESTAMP                                      DEFAULT CURRENT_TIMESTAMP  COMMENT '최종 계산 시간',
    activated       TINYINT                                        DEFAULT 1 NOT NULL          COMMENT '활성화 여부',
    created_at      DATETIME                                       DEFAULT NULL                COMMENT '등록일시',
    updated_at      DATETIME                                       DEFAULT NULL                COMMENT '수정일시',
    created_user_id BIGINT                                         DEFAULT NULL                COMMENT '생성자',
    ...
    FOREIGN KEY (product_id) REFERENCES ad_products_meta(product_id),
    UNIQUE KEY uk_campaign_product_placement (campaign_id, product_id, placement),
    INDEX idx_placement_category (placement, category_path),
    INDEX idx_ad_rank_score (ad_rank_score DESC),
    INDEX idx_exposure_rank (exposure_rank),
    INDEX idx_calculation_status (calculation_status),
    INDEX idx_calculated_at (calculated_at)
) ENGINE=InnoDB COMMENT='AD_RANK 계산 결과 - 실시간 랭킹 조회 및 엘라스틱서치 동기화';
```
...
### 11. 실시간 성과 메트릭 (ad_performance_metrics_realtime)
...
### 12. 이벤트 요약 (ad_event_summaries_daily)
> **목적**: 일별 성과 요약 및 장기 보관
...
### 13. 카테고리별 방어값 통계 (ad_category_defense_stats)
...
### 14. 카테고리 계층 (ad_category_hierarchies)
...
### 15. 엔진 파라미터 (ad_engine_parameters)
...
### 16. 엔진 파라미터 변경 이력 (ad_engine_parameter_histories)
...
| **AD_RANK 계산** | MariaDB + 엘라스틱서치 | AD_RANK 계산 결과, 실시간 랭킹 조회 | 3-7일 |
...
### **이벤트 처리 흐름**
```
실시간: Kafka → Worker → {DB AD_RANK 계산, 엘라스틱서치 동기화}
성과집계: Kafka → Flink → {Redis 지표, DB 집계}
...
ad_campaigns (1) ←→ (N) ad_rank_calculations
ad_products_meta (1) ←→ (N) ad_rank_calculations
```

### **데이터 흐름**
1. **광고 요청** → 엘라스틱서치에서 AD_RANK 랭킹 조회
2. **캠페인 변경** → `ad_rank_calculations` 재계산 → 엘라스틱서치 동기화
3. **클릭 발생** → `ad_impression_pricings`에서 가격 확인 → `ad_balances` 차감
4. **성과 집계** → `ad_performance_metrics_realtime` 업데이트
5. **일별 정산** → `ad_event_summaries_daily` 생성
