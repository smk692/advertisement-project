...
**개발 목표**: AD_RANK 광고 시스템 - Citrus Ad 대체  
**핵심 역할**: 실시간 광고 서빙 + 이벤트 추적 + 어드민 관리  
**성능 목표**: 광고 요청 1-3ms (O(1) 조회), 클릭 처리 30ms 이내  
**기술 스택**: Kotlin + Spring Boot + Elasticsearch + Kafka + PostgreSQL
...
|-----|------------|--------|------|---------------|
| **광고 요청** | `/api/v1/ads/request` | POST | AD_RANK 광고 조회 | 1-3ms (O(1) 조회) |
...
| **잔액 관리** | `/admin/v1/advertisers/{id}/balance` | PUT | 잔액 충전/차감 (선불 방식) | < 100ms |
| **캠페인 등록** | `/admin/v1/campaigns` | POST | 캠페인 등록 (4단계) | < 500ms |
| **캠페인 승인** | `/admin/v1/campaigns/{id}/approve` | PUT | 관리자 승인/반려 | < 200ms |
| **대시보드** | `/admin/v1/dashboard/{advertiserId}` | GET | 성과 대시보드 조회 (실시간 정산) | < 1000ms |
| **정산 데이터** | `/admin/v1/settlement/{advertiserId}` | GET | 실시간 정산 데이터 조회 | < 500ms |
...
처리: 엘라스틱서치 O(1) 조회 → 가격실험 후처리 → 광고 목록 응답
특징: 하이브리드 아키텍처 (15분 배치 + 실시간 이벤트) 기반 조회
```
...
2. 엘라스틱서치 쿼리 생성 및 조회  
   → Index: "ad_rankings" (Alias)
   → Query: placement + category + status 필터링

3. AD_RANK 순위 조회 (하이브리드 아키텍처)
   → 엘라스틱서치에서 정렬된 광고 리스트 조회
   → 필터링: 잔액>0, 승인 캠페인, 예산 여유, 활성 상태
...
5. 주문 취소: 환불 없음 (클릭 시점에 차감 완료)
6. 중복 주문: 첫 번째 주문만 인정 (24시간 내)
...
중복 주문 시나리오:
광고A 클릭(14:00) → 주문1(15:00) → 주문2(16:00) → 주문3(17:00)
→ 주문1만 전환으로 인정 (120,000원 기여 매출)
```
...
3. CTR/CVR 방어값 사용:
   - 기본 방어값: 0.1% (정책 확정)
   - 자동 품질 관리: 없음 (수동 관리 방식)
4. Redis에 방어로직 적용 기록: defense:applied:{campaign_id}

품질 관리 정책:
- 자동 중단 기준: 없음 (광고주 자율성 보장)
- 품질 점수: AD_RANK 알고리즘만 사용
