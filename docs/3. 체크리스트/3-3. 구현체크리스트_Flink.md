...
> **🎯 핵심 목표**: **역할 분담 최적화** - 성과 집계와 전환 매칭만 Flink, AD_RANK 계산은 Worker  
> **💡 아키텍처**: Flink 2개 Statement (성과 집계 + 전환 매칭) + Worker (AD_RANK 계산 + 실시간 처리)

---

## 📊 **최적화된 역할 분담**

### **🔥 Flink 담당 (성과 집계 + 전환 매칭)**
...
-- 🎯 성과 집계 및 클릭전환율 계산
-- 목적: 5분 윈도우 기반 실시간 성과 데이터 집계
INSERT INTO performance_aggregation
SELECT 
  placement,                    -- 광고 영역 (HOME, CATEGORY, SEARCH 등)
  category,                     -- 상품 카테고리 (electronics, fashion 등)
  
  -- 📊 성과 집계 데이터 생성
  COUNT(*) as total_events,     -- 총 이벤트 수
